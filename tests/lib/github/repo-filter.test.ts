...
const DEFAULT_CAPS = { maxFiles: 300, maxTotalBytes: 5_000_000, maxFileBytes: 300_000 };
...
describe("filterScannableFiles", () => {
  it("keeps ordinary source files", () => {
    ...
    expect(result.selected.map((e) => e.path)).toEqual(["src/index.ts", "README.md"]);
  ...
  it("skips a single file over the per-file byte cap", () => {
    ...
    const result = filterScannableFiles(entries, { ...DEFAULT_CAPS, maxFileBytes: 300_000 });
  ...
  it("stops once the max file count is reached and reports truncation", () => {
    ...
    const result = filterScannableFiles(entries, { ...DEFAULT_CAPS, maxFiles: 3 });
  ...
  it("respects the total byte budget, skipping files that would exceed it but keeping later smaller ones", () => {
    ...
    const result = filterScannableFiles(entries, { ...DEFAULT_CAPS, maxTotalBytes: 4100 });

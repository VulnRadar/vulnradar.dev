...
export function CrawlPagesInfo({ crawlInfo, onSelectIssue }: CrawlPagesInfoProps) {
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/50 overflow-hidden">
      ...
      >
        ...
        <span className="text-sm font-semibold text-foreground flex-1">Also Crawled</span>
              ...
              <div key={page.url}>
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-xs text-muted-foreground truncate">{getPath(page.url)}</p>
                    <p className="text-xs text-muted-foreground truncate">{page.url}</p>
                  </div>
                  <div className="flex items-center gap-2 ml-2 shrink-0">
                    <span className="text-xs font-medium text-foreground">{page.findings_count}</span>
                ...
                {expandedPage === page.url && page.findings.length > 0 && (
                  <div className="bg-muted/20 px-4 py-2 space-y-1">
                    {page.findings.map((finding) => (
                      ...
                      >
                        <div className={`w-1.5 h-1.5 rounded-full shrink-0 mt-1 ${SEV_DOT[finding.severity]}`} />
                        <span className="font-medium truncate flex-1">{finding.title}</span>

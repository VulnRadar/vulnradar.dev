...
export function TeamMemberScans({
  ...
  // bg-muted/20, not bg-card: this panel is a drill-down opened from a row of
  // the members panel above it, so it reads as subordinate to it rather than
  // as a fourth peer of identical weight.
  return (
    <div className="overflow-hidden rounded-xl border border-border/50 bg-muted/20">
      <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-border">
        <div className="min-w-0">
          <p className="text-sm font-medium truncate">
            {member.name || member.email}&apos;s scans
          </p>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center px-5">
          <p className="text-sm text-muted-foreground">
            {member.name || member.email} has not run a scan under this team
            yet. Scans they start while signed in show up here.
          </p>
        </div>
      ) : (
        <>

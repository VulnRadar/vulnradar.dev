 ...
 * PaymentFormSkeleton -- see that file's comment for why this stays its
 * own copy instead of a shared import.
 */
...
/**
 * Confirms a GitHub credit purchase directly against Stripe via the
 * confirmGithubCreditPurchase server action -- mirrors
 * ai-credit-checkout.tsx's useConfirmAiCreditPurchase exactly, same retry
 * schedule and same reasoning.
 */
function useConfirmGithubCreditPurchase(
...
function CheckoutForm({
  ...
  const { status, tokens, balance, run } =
    useConfirmGithubCreditPurchase(onSuccess);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let paymentIntent: Awaited<
      ReturnType<typeof createGithubCreditPaymentIntent>
    >;
    try {
      paymentIntent = await createGithubCreditPaymentIntent(tierId);
    } catch (err) {
...
export function GithubCreditCheckout({
  tier,
  onSuccess,
}: {
  tier: GithubCreditTier;
  onSuccess?: (tokens: number, balance: number) => void;
}) {
  ...
  // Mirrors ai-credit-checkout.tsx's two Appearance API themes exactly --
  // copied, not imported, same reasoning as PaymentFormSkeleton above.

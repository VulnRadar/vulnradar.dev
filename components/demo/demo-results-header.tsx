...
import { CheckCircle, AlertTriangle, RefreshCw, ArrowRight, LayoutDashboard } from "lucide-react"
...
export function DemoResultsHeader({ result, onScanAgain }: DemoResultsHeaderProps) {
  ...
  const failedChecks = result.findings.filter(f => f.severity !== "info").length
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-12 pb-8 sm:pt-16">
        <div className="text-center max-w-2xl mx-auto">
          ...
          >
            ...
            {isPassing ? "All checks passed" : `${failedChecks} issue${failedChecks > 1 ? "s" : ""} found`}
          </Badge>
          ...
          <p className="text-base text-muted-foreground mb-6">
            {APP_NAME} completed {result.findings.length} checks in {(result.duration / 1000).toFixed(1)}s
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
            <div className="flex items-center gap-4 px-4 py-2 rounded-lg bg-muted/50 border border-border/50 text-sm">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-medium">{result.findings.length - failedChecks}</span>

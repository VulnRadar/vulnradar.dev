...
export function CheckoutModal({ planId, userId, billing, onClose, onSuccess }: CheckoutModalProps) {
  const selectedPlan = PLANS.find(p => p.id === planId)
  const planName = planId.includes('elite') ? 'Elite' : planId.includes('pro') ? 'Pro' : 'Core'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-background border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden animate-scale-in">
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-4 border-b border-border/60">
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-muted flex-shrink-0" onClick={onClose}>
            <X className="h-4 w-4" />
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          <div className="p-5">
            <StripeCheckout productId={planId} userId={userId} onSuccess={onSuccess} />
          </div>
        </div>

...
export function DocsHero({ id = "overview", badge, title, description, stats, className }: DocsHeroProps) {
  return (
    <section id={id} className={cn("scroll-mt-24", className)}>
      <Badge variant="outline" className="mb-3 sm:mb-4 text-primary border-primary/30 bg-primary/5">
        {badge}
      </Badge>
      ...
      {stats && stats.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-6 sm:mt-8">
          {stats.map((stat, i) => (
            <div key={i} className="flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-card/50 border border-border/50">
              <div>

...
export function getSeverityInfo(share: Share) {
  ...
  if (rating === "unsafe")
    return { label: "Critical", color: "text-red-500", bg: "bg-red-500/10", icon: AlertTriangle }
  if (rating === "caution")
    return { label: "Caution", color: "text-yellow-500", bg: "bg-yellow-500/10", icon: AlertTriangle }

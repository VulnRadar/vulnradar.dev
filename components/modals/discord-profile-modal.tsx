...
export function DiscordProfileModal() {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      {/* flex-col with a scrolling body, the same shape
          components/scanner/share-modal.tsx documents: overflow-hidden
          cancels DialogContent's own overflow-y-auto, so with the header, the
          80px avatar, the sync rows and two h-11 buttons this ran past the
          height cap on a short viewport and "Sync selected" / "Skip for now"
          were unreachable with no way to scroll to them. */}
      <DialogContent className="sm:max-w-[440px] p-0 overflow-hidden border-0 gap-0 flex flex-col max-h-[85vh]">
          ...
          <div className="px-6 pt-4 pb-6">
            ...
            <div className="flex flex-col gap-2">
              <Button
                onClick={handleSync}
                disabled={loading}
                className="relative w-full h-11 bg-[#5865F2] hover:bg-[#4752C4] text-white font-medium"
              >
                {loading ? (
                  <>
                    <Loader2

...
export function SiteModal({
  ...
  /* ModalShell brings the role="dialog", aria-modal, focus trap, Escape and
     background inert this overlay used to have none of (SC 2.4.3, 2.4.11,
     4.1.2), and it always renders a close chip.

     That last part is the one deliberate behaviour change. A notification
     published with is_dismissible false used to hide the X and ignore a scrim
     click, but Escape already routed to handleClose so the announcement was
     never actually inescapable: is_dismissible only ever decided whether a
     dismissal cookie gets written, and handleClose still respects it, so a
     non-dismissible notice still comes back on the next load. What changes is
     that a pointer user now has the same way out a keyboard user always had,
     instead of being sealed in front of it (SC 2.1.2).

     `title` is not optional in practice: POST /api/v3/admin/notifications
     rejects a notification with an empty title or message, so the old branch
     that moved the accessible name onto the message paragraph could not fire
     and is gone. */
  const hasActions = !!(notification.action_url || notification.action_url_2);
  ...
  return (
    ...
    >
      {/* The message is admin-authored and of arbitrary length. The band it
          sits in is the scroller, so a long announcement no longer pushes the

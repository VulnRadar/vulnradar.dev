...
// number -- 795 -> 795 (already a multiple of 5), 793 -> 790. Used for both
// the "N+ CHECKS" label baked into config and the og-image text/filename, so
// a check added or removed can never leave either one claiming more than the
// build actually has.
function roundDownForLabel(n) {
  return Math.floor(n / 5) * 5;
}

// Regenerates public/og-image-<N>.svg/.png from whatever og-image-*.svg
// currently exists, swapping only the "<old>+ CHECKS" text for the new
// rounded label -- the rest of the template (logo, gradient, other two
// badges) is untouched. No-ops if the label hasn't changed, so a build
// with no new checks doesn't re-encode the PNG for nothing. Deletes the
// old numbered svg/png pair so stale ones don't pile up in the repo.
// Returns the current (possibly just-updated) image path for
// CONFIG_SEO_OG_IMAGE, e.g. "/og-image-795.png".
async function regenerateOgImage(roundedLabel) {
  const existingSvg = readdirSync(PUBLIC_DIR).find((f) =>
    /^og-image-\d+\.svg$/.test(f),
  );
  if (!existingSvg) {
    console.warn(
      "[compile-checks-knowledge] no public/og-image-<N>.svg found -- skipping og-image regeneration.",
    );
    return null;
  }

  const svgPath = join(PUBLIC_DIR, existingSvg);
  const svg = readFileSync(svgPath, "utf8");
  const match = svg.match(/(\d+)\+\s*CHECKS/);
  const oldLabel = match ? Number(match[1]) : null;
  const newPngName = `og-image-${roundedLabel}.png`;

  if (oldLabel === roundedLabel) return `/${newPngName}`; // already up to date

  const newSvg = svg.replace(/\d+\+\s*CHECKS/, `${roundedLabel}+ CHECKS`);
  const newSvgName = `og-image-${roundedLabel}.svg`;
  const newSvgPath = join(PUBLIC_DIR, newSvgName);
  const newPngPath = join(PUBLIC_DIR, newPngName);

  writeFileSync(newSvgPath, newSvg, "utf8");
  await sharp(Buffer.from(newSvg)).png().toFile(newPngPath);

  if (newSvgName !== existingSvg) unlinkSync(svgPath);
  const oldPngPath = join(PUBLIC_DIR, `og-image-${oldLabel}.png`);
  if (existsSync(oldPngPath) && oldPngPath !== newPngPath) {
    unlinkSync(oldPngPath);
  }

  console.log(
    `[compile-checks-knowledge] og-image: ${oldLabel ?? "?"}+ -> ${roundedLabel}+ (${newSvgName} + ${newPngName})`,
  );
  return `/${newPngName}`;
}
...
async function build() {
  ...
  // is added, removed, or a category changes. The marketing "N+" label and
  // the og-image are ALSO regenerated here now (see roundDownForLabel /
  // regenerateOgImage above) -- lib/config/config-values.ts's
  // CONFIG_TOTAL_CHECKS_LABEL / CONFIG_SEO_OG_IMAGE default to these
  // generated values, so neither can silently go stale relative to the
  // real check count the way a hand-typed "750+" eventually did.
  ...
  const ogImagePath = await regenerateOgImage(roundedLabel);
  const statsLines = [
    ...
    ...(ogImagePath
      ? [`export const GENERATED_OG_IMAGE = "${ogImagePath}";`]
      : []),
    "",
  ];

...
function build() {
  ...
  // is added, removed, or a category changes -- only the marketing "N+"
  // label (CONFIG_TOTAL_CHECKS_LABEL) stays a hand-set, deliberately
  // conservative admin setting.
  ...
  const statsLines = [
    ...
    `export const EXACT_CHECK_COUNT = ${totalChecks + pageCheckCount};`,
    `export const EXACT_CHECK_CATEGORY_COUNT = ${files.length};`,
    "",
  ];
  writeFileSync(STATS_OUTPUT, statsLines.join("\n"), "utf8");
  console.log(
    `[compile-checks-knowledge] wrote ${STATS_OUTPUT.replace(ROOT + "\\", "")} (EXACT_CHECK_COUNT=${totalChecks + pageCheckCount})`,
  );

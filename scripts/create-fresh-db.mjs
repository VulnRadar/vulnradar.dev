...
function banner() {
  ...
  log(
    `${c.bold}${c.cyan}  ║   VulnRadar Safe Database Migration          ║${c.reset}`,
  );
  log(
    `${c.bold}${c.cyan}  ║   Creates new DB, preserves original data    ║${c.reset}`,
  );
  log(
    `${c.bold}${c.cyan}  ╚══════════════════════════════════════════════╝${c.reset}`,
  );
...
async function main() {
  ...
  log(
    `${c.bold}Current Database:${c.reset} ${c.cyan}${dbInfo.database}${c.reset} on ${dbInfo.host}`,
  );
  log("");
  ...
  log("");
  log(
    `${c.bold}Original database has ${existingTables.length} tables:${c.reset}`,
  );
  ...
  log(
    `  ${c.green}4.${c.reset} Original database ${c.bold}${dbInfo.database}${c.reset} is ${c.green}NEVER modified${c.reset}`,
  );
  log("");

  const shouldProceed = await askYesNo(
    "Proceed with creating the new database?",
    true,
  );
  if (!shouldProceed) {
  ...
  try {
    ...
    if (existsRes.rows.length > 0) {
      ...
      const shouldDrop = await askYesNo(
        `Drop and recreate '${newDbName}'?`,
        false,
      );
      if (shouldDrop) {
  ...
  const tablesToMigrate = MIGRATE_TABLES.filter(
    (t) => existingTables.includes(t) && tableCounts[t] > 0,
  );
  ...
  if (tablesToMigrate.length === 0) {
    ...
    log(
      `${c.cyan}The following tables have data that can be migrated:${c.reset}`,
    );
    ...
    if (shouldMigrate) {
      ...
      log(
        `${c.dim}Transferring data from original database to new database...${c.reset}`,
      );
      log("");
      ...
      for (const table of orderedTables) {
        ...
        try {
          ...
          const newColumnsInfo = new Map(
            newColRes.rows.map((r) => [r.column_name, r]),
          );
          ...
          for (const [col, defaultVal] of Object.entries(tableDefaults)) {
            if (
              !oldColumns.has(col) &&
              !mappedTargets.has(col) &&
              newColumnsInfo.has(col)
            ) {
          ...
          success(
            `  Migrated ${table}: ${migrated}/${dataRes.rows.length} rows`,
          );
        } catch (err) {
      ...
      try {
        ...
        if (roleUpdate.rowCount > 0) {
          success(
            `  Fixed ${roleUpdate.rowCount} users with invalid 'beta_tester' role -> 'user'`,
          );
        } else {
  ...
  log("");
  log(
    `${c.bold}${c.bgGreen}${c.black}                                                         ${c.reset}`,
  );
  log(
    `${c.bold}${c.bgGreen}${c.black}   Migration Complete!                                   ${c.reset}`,
  );
  log(
    `${c.bold}${c.bgGreen}${c.black}                                                         ${c.reset}`,
  );
  log("");
  log(
    `  ${c.green}✓${c.reset} New database created: ${c.bold}${c.green}${newDbName}${c.reset}`,
  );
  log(`  ${c.green}✓${c.reset} ${created} tables with fresh v2 schema`);
  log(
    `  ${c.green}✓${c.reset} Original database ${c.bold}${dbInfo.database}${c.reset} is ${c.green}safe and untouched${c.reset}`,
  );
  log("");
  ...
  log("");
  log(
    `  ${c.white}2.${c.reset} Restart your dev server to use the new database`,
  );
  log("");
  log(
    `  ${c.white}3.${c.reset} Once verified, you can optionally delete the old database:`,
  );

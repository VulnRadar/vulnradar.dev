...
function getSchemaVersion() {
  try {
    ...
    console.error(
      "ERROR: Could not read version from config.yaml:",
      err.message,
    );
    process.exit(1);
  }
...
function banner() {
  ...
  log(
    `${c.bold}${c.cyan}  ║           Schema v${SCHEMA_VERSION}              ║${c.reset}`,
  );
  log(`${c.bold}${c.cyan}  ╚══════════════════════════════════════╝${c.reset}`);
  log("");
}
...
async function runV2Migration(pool, actual, v1Info) {
  log("");
  log(
    `${c.bold}${c.bgYellow}${c.white} V1 -> V2 MIGRATION DETECTED ${c.reset}`,
  );
  log("");
  ...
  log(
    `  1. Add new columns to the ${c.bold}users${c.reset} table (plan, stripe_customer_id, etc.)`,
  );
  log(
    `  2. Create new tables: ${c.bold}badges${c.reset}, ${c.bold}user_badges${c.reset}, ${c.bold}billing_history${c.reset}`,
  );
  ...
  try {
    await pool.query(
      `ALTER TABLE users ADD COLUMN IF NOT EXISTS email_session_revoked BOOLEAN NOT NULL DEFAULT false`,
    );
  ...
  try {
    await pool.query(
      `ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS daily_limit INTEGER DEFAULT 50`,
    );
  ...
  for (const colDef of notifPrefColumns) {
    try {
      await pool.query(
        `ALTER TABLE notification_preferences ADD COLUMN IF NOT EXISTS ${colDef}`,
      );
    } catch {
      /* column may already exist */
    }
  ...
  try {
    await pool.query(
      `ALTER TABLE users ADD COLUMN IF NOT EXISTS daily_scan_limit INTEGER DEFAULT NULL`,
    );
  ...
  log("");
  log(
    `${c.cyan}Your database has been upgraded to VulnRadar v2 schema.${c.reset}`,
  );
  log(
    `${c.cyan}New features available: badges, billing history, subscription management, admin notifications.${c.reset}`,
  );
  log("");
...
async function runFreshInstall(pool, actual) {
  ...
  log("");
  log(
    `${c.red}${c.bold}WARNING: This will DROP ALL TABLES and create a fresh database!${c.reset}`,
  );
  ...
  if (existingTables.length > 0) {
    ...
    for (const t of existingTables) {
      try {
        ...
        log(
          `  - ${c.bold}${t}${c.reset} ${c.dim}(${countRes.rows[0].total} rows)${c.reset}`,
        );
      } catch {
        log(`  - ${c.bold}${t}${c.reset}`);
      }
  ...
  const shouldProceed = await askDanger(
    "Are you SURE you want to delete ALL data and start fresh?",
  );
  if (!shouldProceed) {
  ...
  const reallyProceed = await askDanger(
    "Type 'y' again to confirm PERMANENT DATA DELETION:",
  );
  if (!reallyProceed) {
    info("Fresh install cancelled.");
    return false;
  ...
  log("");
  success(
    "All tables dropped. Start the app (npm run dev) to create fresh tables.",
  );
  return true;
}
...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    error(
      "DATABASE_URL is not set. Add it to .env.local or set it as an environment variable.",
    );
    process.exit(1);
  }
  ...
  if (v1Info.isV1 || forceV2) {
    if (v1Info.isV1) {
      ...
      log(`${c.yellow}Your database is running VulnRadar v1 schema.${c.reset}`);
      log(
        `${c.yellow}Missing ${v1Info.missingTables.length} tables, ${v1Info.missingColumns.length} columns.${c.reset}`,
      );
      log("");
      ...
      log(
        `${c.cyan}than incremental migration for large schema changes.${c.reset}`,
      );
      log("");
      log(
        `${c.dim}If you have important data to preserve, the migration below will${c.reset}`,
      );
      log(
        `${c.dim}attempt to add missing tables/columns without data loss.${c.reset}`,
      );
      log("");

      const continueAnyway = await askReview(
        "Continue with incremental migration anyway?",
      );
      if (!continueAnyway) {
        log("");
  ...
  if (pendingTableRenames.length > 0) {
    ...
    for (const { oldName, newName } of pendingTableRenames) {
      ...
      const shouldRename = await askReview(
  ...
  for (const [table, renames] of Object.entries(COLUMN_RENAMES)) {
    ...
    for (const [oldCol, newCol] of Object.entries(renames)) {
      if (
        actualTable.includes(oldCol) &&
        !actualTable.includes(newCol) &&
        expectedCols.includes(newCol)
      ) {
  ...
  if (pendingColRenames.length > 0) {
    ...
    for (const { table, oldCol, newCol } of pendingColRenames) {
      log(
        `  ${c.magenta}RENAME${c.reset}  ${c.bold}${table}.${oldCol}${c.reset} ${c.dim}->${c.reset} ${c.bold}${table}.${newCol}${c.reset}`,
      );
  ...
  for (const [table, expectedCols] of Object.entries(expected)) {
    if (!actual[table]) {
      ...
      log(
        `  ${c.red}MISSING TABLE${c.reset}  ${c.bold}${table}${c.reset} ${c.dim}(${expectedCols.length} columns)${c.reset}`,
      );
      continue;
    }
  ...
  if (unknownTables.length > 0) {
    ...
    for (const t of unknownTables) {
      log(
        `  ${c.yellow}EXTRA TABLE${c.reset}    ${c.bold}${t}${c.reset} ${c.dim}(${actual[t].length} columns, not in expected schema)${c.reset}`,
      );
    }
  }
  ...
  if (missingTables.length > 0) {
    ...
    log(
      `${c.dim}These will be created automatically when you start the app (npm run dev).${c.reset}`,
    );
  }
  ...
  if (missingColumns.length > 0) {
    log("");
    log(
      `${c.yellow}${missingColumns.length} column(s) are missing from existing tables.${c.reset}`,
    );
    log(
      `${c.dim}These will be added automatically when you start the app, or you can add them now.${c.reset}`,
    );
    log("");

    const shouldReview = await askReview(
      `Review and add ${missingColumns.length} missing column(s)?`,
    );
    if (shouldReview) {
      for (const { table, column } of missingColumns) {
        ...
        if (alterMatch) {
          const shouldAdd = await askDanger(
            `Add column ${c.bold}${table}.${column}${c.reset}?`,
          );
          if (shouldAdd) {
            try {
              await pool.query(
                `ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${column} ${alterMatch[1].split(";")[0].trim()}`,
              );
          ...
          warn(
            `Could not find column definition for ${table}.${column}. Start the app to auto-migrate.`,
          );
        }
      }
    }
  ...
  if (extraColumns.length > 0) {
    log("");
    log(
      `${c.yellow}${extraColumns.length} extra column(s) found that are not in the current schema.${c.reset}`,
    );
    log(
      `${c.dim}These may be from custom modifications or older versions.${c.reset}`,
    );
    log(
      `${c.red}${c.bold}WARNING: Dropping columns permanently deletes data!${c.reset}`,
    );
    log("");
    ...
    if (shouldReview) {
      for (const { table, column } of extraColumns) {
        ...
        const shouldDrop = await askDanger(
          `Drop column ${c.bold}${table}.${column}${c.reset}? THIS CANNOT BE UNDONE.`,
        );
        if (shouldDrop) {
          try {
  ...
  if (unknownTables.length > 0) {
    log("");
    log(
      `${c.yellow}${unknownTables.length} table(s) found that are not part of the VulnRadar schema.${c.reset}`,
    );
    log(
      `${c.dim}These may be from custom modifications, plugins, or a shared database.${c.reset}`,
    );
    log(
      `${c.red}${c.bold}WARNING: Dropping tables permanently deletes ALL data!${c.reset}`,
    );
    log("");
    ...
    if (shouldReview) {
      for (const table of unknownTables) {
        log("");
        log(
          `  Table: ${c.bold}${table}${c.reset}  ${c.dim}(${actual[table].length} columns)${c.reset}`,
        );
        ...
        const shouldDrop = await askDanger(
          `Drop table ${c.bold}${table}${c.reset}? THIS CANNOT BE UNDONE.`,
        );
        if (shouldDrop) {
          const reallyDrop = await askDanger(

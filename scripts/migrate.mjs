...
async function runV2Migration(pool, actual, v1Info) {
  ...
  // Step 14: Create password_rotation_policies table
  if (!actual["password_rotation_policies"]) {
    info("Creating password_rotation_policies table...")
    try {
      await pool.query(`
      ...
      `)
      success("  Created password_rotation_policies table")
    } catch (err) {
      error(`  Failed to create password_rotation_policies table: ${err.message}`)
    }
  }

  // Step 15: Create user_password_requirements table
  if (!actual["user_password_requirements"]) {
    info("Creating user_password_requirements table...")
    try {
      await pool.query(`
      ...
      `)
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_user_password_requirements_policy ON user_password_requirements(policy_id)`)
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_user_password_requirements_next_change ON user_password_requirements(next_required_change)`)
      success("  Created user_password_requirements table")
    } catch (err) {
      error(`  Failed to create user_password_requirements table: ${err.message}`)
    }
  }

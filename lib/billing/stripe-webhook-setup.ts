...
import { REQUIRED_EVENTS } from "./stripe-webhook-events";
...
// The list itself lives in ./stripe-webhook-events, which carries no
// `server-only` marker and no Stripe/database imports, so /docs/self-hosting
// can render it instead of keeping a hand-written copy that goes stale. See
// that file's header.
export { REQUIRED_EVENTS };

// Cache to avoid repeated API calls
let webhookSetupPromise: Promise<{
  success: boolean;
  webhookId?: string;
  error?: string;
}> | null = null;

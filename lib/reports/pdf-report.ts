...
export function generatePdfReport(result: ScanResult): Uint8Array {
  ...
  for (const sev of severities) {
    ...
    addText(
      `  ${sev.toUpperCase().padEnd(10)} ${String(count).padStart(3)}${bar}`,
      10,
      count > 0,
      count > 0 ? SEVERITY_COLORS[sev] : TEXT_MUTED,
    );
    addSpacer(2);
  }

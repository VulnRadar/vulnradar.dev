...
export const CONFIG_APP_VERSION = "4.0.0";
...
// also sends the protocol-relative bypass and resolves Location like a
// browser. Dedupe now runs over the whole scan, async findings included, and
// merges per component; the OSV.dev check reports one finding per library
...
// The ?v= is a cache key, not decoration. /og-image.png is a stable path, so
// every cache that keys on the URL kept serving whatever bytes it fetched
// first: the card changed on 31 August and Discord was still showing the old
// one two weeks later. Discord's ~2 hour refresh re-scrapes the link's
// metadata, but the image itself is proxied through media.discordapp.net and
// cached far longer, and Cloudflare caches PNGs at the edge on top of that.
// The hash is the image's own content, regenerated by
// scripts/assets/build-og-image.mjs, so the URL changes exactly when the
// picture does and never otherwise.
export const CONFIG_SEO_OG_IMAGE = `/og-image.png?v=${OG_IMAGE_HASH}`;

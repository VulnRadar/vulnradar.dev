...
export const SETTINGS_REGISTRY = {
  ...
  DOMAIN_REVERIFY_ENABLED: {
    tier: "runtime",
    type: "bool",
    default: CONFIG_DOMAIN_REVERIFY_ENABLED,
    group: "Features",
  ...
  DOMAIN_REVERIFY_INTERVAL_DAYS: {
    tier: "runtime",
    type: "int",
    default: CONFIG_DOMAIN_REVERIFY_INTERVAL_DAYS,
    group: "Features",
    label: "Domain re-verify staleness window (days)",
    help: "A verified domain is re-checked once it's been this many days since its last check.",
    min: 1,
    max: 365,
  },
  DOMAIN_REVERIFY_BATCH_SIZE: {
    tier: "runtime",
    type: "int",
    default: CONFIG_DOMAIN_REVERIFY_BATCH_SIZE,
    group: "Features",
    label: "Domain re-verify batch size",
    help: "Caps how many domains one worker tick re-checks, so a large domains table can't turn a single tick into an unbounded burst of DNS lookups.",
    min: 1,
    max: 1000,
  },
  ...
  AI_REASONING_TIMEOUT_MULTIPLIER: {
    tier: "runtime",
    type: "int",
    default: CONFIG_AI_REASONING_TIMEOUT_MULTIPLIER,
    group: "AI",
    label: "Reasoning-model timeout multiplier",
    help: "Reasoning models spend most of their time before the first token, so every AI timeout below is multiplied by this when the selected model will think. Raise it if verdicts, summaries or tag suggestions started going missing after switching to a thinking model.",
    min: 1,
    max: 20,
  },
  ...
  ENGINE_FEEDBACK_NOISE_THRESHOLD_PERCENT: {
    ...
    help: "The Admin Engine Feedback panel flags a check whose false-positive rate, or an auto-tag rule whose dismissal rate, is at or above this percentage -- provided it also clears the minimum sample size below. Reporting only for auto-tag rules; for checks, this same threshold also gates adaptive confidence discounting (see Adaptive confidence scoring below).",
    min: 1,
    max: 100,
  },
  ENGINE_FEEDBACK_MIN_SAMPLE_SIZE: {
    ...
    help: "Feedback submissions (per check) or rule firings (per auto-tag rule) required before the Admin Engine Feedback panel will flag it as noisy, so one user's feedback on one scan can't flag a check on its own. Also gates adaptive confidence discounting for checks (see below).",
    min: 1,
    max: 10000,
  },
  ADAPTIVE_CONFIDENCE_ENABLED: {
    tier: "runtime",
    type: "bool",
    default: CONFIG_ADAPTIVE_CONFIDENCE_ENABLED,
    group: "Advanced",
    label: "Adaptive confidence scoring",
    help: "Automatically discounts a finding's confidence when its check is flagged by the two engine-feedback settings above (a real, statistically meaningful false-positive rate from user feedback). Off falls back to each check's static, hand-set confidence forever.",
  },

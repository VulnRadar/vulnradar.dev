  ...
  },
  {
    ...
    changes: [
      ...
      },
      {
        icon: CheckCheck,
        label:
          "CI Builds the Real Image, Boots It, and Waits for the Test Suite",
      ...
      },
      {
        icon: Lock,
        label:
          "More Internal Address Ranges Blocked, and Every Redirect Re-Checked",
        desc: "The scanner's private-address block list was missing the shared address space used by carrier-grade NAT and by Kubernetes pod networks, which left Alibaba Cloud's instance metadata endpoint reachable while the equivalent AWS one was blocked, and three of the IPv6 rules covered only the first slice of the range they were meant to cover, so some private and link-local addresses used by container and overlay networks were not caught. Those are blocked now, along with the benchmarking, protocol-assignment and documentation ranges, in both notations, with tests pinning each boundary. The safety check every part of the scanner runs before a request also looked at the address but never the protocol, so a redirect could hand it something that was not a web address at all, and only the original URL was vetted rather than each hop; every redirect step is now re-checked against the same rule. Subdomain discovery pulls from nine free public services, and those requests followed redirects blindly and read whatever came back with no size limit, so an outage or hijack at one of them could have pointed the scanner somewhere internal or streamed a huge response into memory. And the optional page screenshot only checked the address you asked for, so a page that redirected the browser somewhere the scanner would never have been allowed to visit was still captured and saved.",
        category: "security",
      },
      {
        icon: Zap,
        label: "Scans Send About Half the Requests They Used To",
      ...
      {
        icon: CreditCard,
        label: "Billing Bugs That Downgraded the Wrong Account",
      ...
      {
        icon: Lock,
        label: "Sign-In, Sessions and Two-Factor Hardened",
      ...
      },
      {
        icon: Network,
        label:
          "Webhook Secrets Are Encrypted, Rotatable, and Deliveries Are Visible",
      ...
      {
        icon: Gauge,
        label: "Lists Stopped Loading Every Finding to Draw a Badge",
      ...
      {
        icon: Puzzle,
        label: "The Browser Extension Can Point at Your Own Instance",

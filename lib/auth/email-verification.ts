 ...
 * verification link (fire-and-forget). Stores hashAuthToken(token) (see
 * lib/auth/token-hash.ts) so verify-email matches on the hash, never the raw
 * token, and clears any prior tokens for the user first so only the newest
 * link is live.
 *

...
export const featureSchemaSteps = [
  ...
  // Team pictures, stored the same way user avatars are. No teams.avatar_url
  // to go with it: a user avatar can be an external OAuth URL and so needs a
  // column, but a team's is always these bytes or nothing, so the URL is
  // derived from this row's updated_at and the two cannot drift apart.
  // The primary key serves both reads: the single-team fetch and the
  // team_id = ANY($1::int[]) decoration the list does.
  {
    id: "team-avatars",
    onError: "warn",
    warning: "Failed to create/verify team_avatars",
    sql: `
  ...
  // ════════════════════════════════════════════════════════════════
  // SCHEDULED SCANS — consecutive failure counter.
  //
  // A scheduled scan that fails (target down, DNS gone, TLS broken, timed
  // out) told nobody. The worker only emailed on `status === "completed"`,
  // so a schedule could fail every hour for a month in silence while the
  // owner believed their site was being watched. This counter is what lets
  // the worker email the FIRST failure and then stay quiet until a run
  // succeeds again: an hourly schedule against a dead host sends one email,
  // not twenty-four a day.
  // ════════════════════════════════════════════════════════════════
  {
    id: "scheduled-scans-consecutive-failures",
    onError: "warn",
    warning: "Failed to add scheduled_scans.consecutive_failures",
    sql: `
        ALTER TABLE scheduled_scans
  ...
  // The index behind the directory's own query. /public-scans is
  // unauthenticated, rate-limited at 60/min, and linked from the sitemap, so
  // it is the one listing a crawler hits hardest, and nothing supported its
  // filter: idx_scan_history_scanned_at orders the whole table and then
  // discards every row that is not a live public share, which on an instance
  // where most scans are private means reading a long way down scan_history to
  // find one page of twenty. The COUNT(*) beside it re-read the same rows
  // again on every request.
  //
  // Partial, so it holds only the rows the directory can ever show rather than
  // a second copy of every scan ever run. scanned_at DESC is the directory's
  // own ORDER BY, so the same index answers the sort and the LIMIT without a
  // sort step.
  //
  // The expiry test stays out of the predicate on purpose: it compares against
  // NOW(), and a partial index predicate has to be immutable, so a time-varying
  // one is not just disallowed, it would be wrong the moment it was built. It
  // remains a residual filter, now over a small candidate set instead of the
  // whole table.
  {
    id: "scan-history-public-directory-index",
    onError: "warn",
    warning: "Failed to add the public scans directory index",
    sql: `
        CREATE INDEX IF NOT EXISTS idx_scan_history_public_listed

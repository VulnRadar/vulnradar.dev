...
export const coreSchemaSteps = [
  ...
  // ════════════════════════════════════════════════════════════════
  {
    ...
    sql: `
        CREATE TABLE IF NOT EXISTS users (
          ...
          -- answers in (lib/i18n/config.ts). NULL means the account has never
          -- chosen, so the browser decides.
          locale VARCHAR(10),
  ...
  //
  // `redacted_html` is the rendered document nodemailer was actually
  // handed, put through the same redaction (redactEmailHtml). It exists
  // because redacted_preview is the plain-text alternative part, and the
  // admin viewer was pouring that 500-character excerpt into the branded
  // shell and presenting the result as the message that went out. It was
  // not: no heading, no button, no detail panel, and every link reading
  // [REDACTED LINK]. Storing the real markup is what lets the viewer show
  // the message instead of a mock-up of it. NULL on rows written before
  // this column existed, and on a message too large to keep whole.
  // ════════════════════════════════════════════════════════════════
  {
    id: "email-logs",
    onError: "warn",
    ...
    sql: `
        ...
        -- Gives the column to databases that already have this table:
        -- CREATE TABLE IF NOT EXISTS above no-ops for them, and the
        -- INSERT in lib/email/email.ts names the column unconditionally.
...
CREATE INDEX IF NOT EXISTS idx_access_rules_active ON access_rules(is_active,
  ...
  // ════════════════════════════════════════════════════════════════
  // BROADCAST TEMPLATES - Reusable starting points an admin can write
  // ════════════════════════════════════════════════════════════════
  //
  // lib/email/campaigns.ts holds seven written campaign templates, and they
  // are a compiled-in `const` array imported straight into a client
  // component. Adding an eighth means editing that file and shipping a
  // release, which is a developer action, so the answer to "can we add our
  // own template" was literally no.
  //
  // This is the other half: a template an admin writes in the composer and
  // saves. Deliberately NOT a data version of the built-ins. Those carry typed
  // fields and subject/body FUNCTIONS, and turning them into rows would mean
  // inventing a placeholder language and an editor for it. A saved template is
  // the simpler thing people actually asked for: a subject and a body you
  // start from and then edit, which is how the built-ins are used in practice
  // once their fields are filled.
  {
    ...
    sql: `
        CREATE TABLE IF NOT EXISTS broadcast_templates (
          ...
          content TEXT NOT NULL,
          -- SET NULL, not CASCADE: a template is shared staff property and
          -- outlives whoever typed it. Deleting a departing admin's account
          -- must not silently delete the announcement template the team uses.
          created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
          created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
          updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
        );
        -- Case-insensitively unique, which is what makes saving an upsert:
        -- saving "October promo" twice updates it rather than leaving two
        -- entries in the picker that differ only in capitalisation.
  ...
  // ════════════════════════════════════════════════════════════════
  // ACCOUNT LANGUAGE — the language this account reads the site, its
  // emails and its AI answers in (lib/i18n/config.ts). NULL until the
  // account picks one, which is what leaves the browser's own
  // Accept-Language in charge.
  // ════════════════════════════════════════════════════════════════
  {
    id: "users-locale",
    onError: "warn",
    warning: "Failed to add users.locale",
    sql: `
        ALTER TABLE users

...
export const integrationSchemaSteps = [
  ...
  // Contact-form submissions, stored because emailing them was the only
  // record there was.
  //
  // Both contact routes built two emails, fired them with a void inside a
  // queueMicrotask, caught any failure into a console.error, and returned
  // "Thanks for reaching out. We will get back to you soon." A DNS failure at
  // the mail host, a bad SMTP credential, a bounce or a spam filter therefore
  // lost the message outright: the sender was told it arrived and nobody here
  // ever knew it existed. The security category makes that worse than a lost
  // sales lead.
  //
  // No user_id: the landing form takes submissions from people with no
  // account, and the account form's sender is identified by the address they
  // typed rather than their session, which is what support replies to. The
  // address is stored in clear for the same reason, and account deletion
  // purges by recipient the way email_logs already does.
  {
    id: "contact-submissions",
    onError: "warn",
    warning: "Failed to create/verify contact_submissions",
    sql: `
        CREATE TABLE IF NOT EXISTS contact_submissions (
          ...
          -- 'pending' until the send settles, so a row still pending long
          -- after its created_at means the process died mid-send.
        ...
        -- The query staff actually run: what has come in that nobody has
        -- picked up, and what failed to reach us by mail.
  ...
  // while a whole ticket CASCADE-deletes with its owner.
  {
    ...
    sql: `
        ...
        CREATE TABLE IF NOT EXISTS support_ticket_messages (
          ...
          -- body is always exactly what the author typed; body_locale is the
          -- language they wrote it in, and translations caches the message in
          -- other languages so each side reads the thread in their own.
          body_locale VARCHAR(10),
  ...
  // ════════════════════════════════════════════════════════════════
  // SUPPORT ACROSS A LANGUAGE BARRIER — the language a ticket message was
  // written in, and that message cached in the languages the other side
  // reads (lib/support/translate.ts). body itself is never touched: it
  // stays exactly what the author typed.
  // ════════════════════════════════════════════════════════════════
  {
    id: "support-ticket-message-translations",
    onError: "warn",
    warning: "Failed to add support_ticket_messages translation columns",
    sql: `
        ALTER TABLE support_ticket_messages
          ADD COLUMN IF NOT EXISTS body_locale VARCHAR(10);
        ALTER TABLE support_ticket_messages

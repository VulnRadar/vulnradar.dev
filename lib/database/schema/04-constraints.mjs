...
export const VALUE_SET_CHECKS = [
  ...
  {
    // The one this list was missing, and arguably the most load-bearing value
    // set in the schema. scan_history.status drives the pending/running queue
    // view, the stale-scan sweep, the badge, the public-scan feed and every
    // "is this scan finished" branch in lib/scanner/scan-jobs.ts. The four
    // values match ScanJobStatus in lib/scanner/types.ts, which is the type
    // every writer is checked against at compile time - but a raw UPDATE, a
    // migration or a future writer that builds the string rather than
    // importing the type had nothing stopping it at the database.
    table: "scan_history",
    name: "scan_history_status_check",
    expr: "status IN ('pending', 'running', 'completed', 'failed')",
  },
];

...
export function tokenize(text) {
  ...
  for (const raw of matches) {
    ...
    // A version with and without its "v". The changelog's headings are
    // "v3.8.0 - September 3, 2026" and people ask "what changed in 3.8.0",
    // which are two different tokens: without this the release a question
    // names by number is the one release it cannot find.
    const version = /^v(\d[\d.]*)$/.exec(raw);
    if (version) terms.push(version[1]);

...
change with its category tag and full description. Kinds of change:
breaking, security, added, improved, fixed, changed, performance,
removed, deprecated. Product areas (used from 4.0.0): engine, api,
admin, cli, extension, accessibility, selfhost.
...
### Changes
- [RefreshCw] **[FIXED]** **Fixed the Update Button Wrongly Disabled on Some Self-Hosts**
  Self-hosted installs running inside certain hosting panels, such as Pterodactyl, Pelican, or unRAID, lost their in-app Update button, even though these setups had always been able to update themselves that way. This happened because a fix meant for VulnRadar's own official Docker image, which cannot update itself in-app, was applied too broadly to anything that looked like it ran in a container. VulnRadar now actually tests whether it can write an update to the install, rather than guessing from how it runs, so only installs that truly cannot self-update are told to fetch a new image instead.
- [Package] **[CHANGED]** **Self-Host Updates No Longer Install Files You Don't Need**
  When a self-hosted install updates itself, it copies over the new release and is meant to skip files that only matter to developers. That skip list had gone stale, so every update also installed a separate command-line tool package, code-formatting configuration, Docker build instructions, and internal notes about how our own hosted version is deployed, none of which a running install needs. These are now correctly left out of every future update, including for installs that already picked them up. Files people are actually told to use, like the README, are kept.
- [Database] **[SECURITY]** **An Encrypted Backup Was Accidentally Published**
  A database backup created by the admin panel was accidentally saved into VulnRadar's source code and briefly shipped inside the download for version 3.8.1. The backup was strongly encrypted and its decryption key was never stored near it, so the file could not be read by anyone who obtained it, though it should not have been published regardless. The rule stopping backups being saved into the source code only covered one backup process and missed the admin panel's own; both are covered now, the file is removed, and an automatic check stops this happening again.
- [Container] **[FIXED]** **Fixed Our Setup File Pointing to an Older Version**
  The Docker setup file we publish for self-hosting names a specific version of VulnRadar to install, and that name has to be updated by hand with every release. After version 3.8.1 shipped, the file was still naming the previous version, 3.8.0, so anyone following our own setup instructions would install an older release with no sign that anything was wrong. This is now checked automatically, so a release cannot go out with the setup file pointing at the wrong version.
...
- [Search] **[FIXED]** **Fixed a Search Engine Confusion About Our Homepage Address**

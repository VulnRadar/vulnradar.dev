...
_Auto-compiled from `lib/changelog/data.ts` on 2026-09-07._
...
- [Timer] **[FIXED]** **Every AI Timeout Was Set for a Fast Model**
  Each AI feature waited a fixed number of seconds for a reply, and every one of those numbers was chosen against a quick non-reasoning model, because that is what our managed deployment ran. A model that thinks before answering spends most of its time before the first token appears, so it overran all of them, and each feature then failed silently in its own way: a finding came back with no verdict rather than an uncertain one, a scan summary was quietly dropped, a tag suggestion simply never showed up. Nothing logged an error. The only symptom was output getting thinner after somebody changed the model, which is close to impossible to diagnose from the outside. Auto-tagging was the worst of them at twelve seconds, which no thinking model will ever meet. Every AI timeout is now multiplied when the selected model will actually reason, whether it reasons because we asked through the Anthropic body, because we asked with reasoning_effort, or because it is one of the models that always does. The multiplier is a single admin setting rather than a second set of hardcoded numbers, so the fast-model values stay the ones an operator tuned and the reasoning allowance moves with them; setting it to 1 turns the whole behaviour off. The deep-scan batch budget went up to match, because raising a per-call ceiling without raising the batch would have cut the number of findings a large scan gets through, and the API routes' own ceilings moved above that in turn.
- [Container] **[CHANGED]** **Self-Hosting Note: Your Reverse Proxy Has a Timeout Too**
...
- [Package] **[CHANGED]** **Updates Stopped Installing Things a Running Copy Has No Use For**
  An update copies the release over your install and removes what the release no longer ships, and the list of what it skips had grown stale: test suites, CI config and the browser extension were dropped, and everything else went in. So every install carried the command-line tool, which is a separate package on npm, the ESLint and Prettier and shadcn configs, the Dockerfile and its ignore file, which only matter to someone building the image rather than running it, and the contributor documents plus the note describing how vulnradar.dev itself is deployed, which is not a fact about anyone else's server. All of it is dropped now, on the next update, including from installs that already have it. What stays, stays for a reason that is written down beside it: the README and SECURITY.md because our own self-hosting page sends you to them by name in your install directory, .env.example because it is the file those docs tell you to copy, the compose files because they are a documented way to run the database, and .gitignore because deleting it from an install that was cloned rather than unpacked turns your .env into a file git will happily let you commit. A test now reads the repository root and fails if anything in it is in neither list, so the next file added to the top of the tree is a decision rather than a default.
- [Database] **[SECURITY]** **An Encrypted Database Backup Was Committed to the Repository**
  A backup written by the admin panel was committed and shipped inside the v3.8.1 source tarball. The dump is AES-256-GCM encrypted and the key lives only in the environment, never in the repository, so it is not readable by anyone who has the file; the sidecar beside it holds the algorithm, the nonce and the authentication tag, and no key material. It should not have been there regardless. The only ignore rule for backups covered the pre-migration dumps written by the migration tooling, and the admin panel and the backup script write somewhere else entirely, which nothing covered. Both directories are ignored now, along with the dump file shapes wherever they are written from, so this cannot be committed from a path nobody thought of. The file is gone from the tree, and a test fails the build if a dump ever appears in it again.
- [Container] **[FIXED]** **Our Own Compose File Installed the Previous Release**
  docker-compose.yml pins the image tag by hand and nothing bumped it, so it still named v3.8.0 after v3.8.1 shipped. That is the most-copied file we publish, and a stale pin is invisible: the file looks right, the pull succeeds, and the app starts, just a version behind for everyone who followed the documentation. The pin is now checked against the version of the tree it sits in, so a release cannot go out naming a different one.
...
- [Lock] **[FIXED]** **A Self-Host Following Our Own Setup Guide Could Never Log In**
  Step two of the README is to copy the example environment file, and that file shipped placeholder mail-server credentials switched on. Everything here treats a setting as configured the moment it is not empty, so the app believed it could send mail, stopped auto-verifying new accounts, and waited for a verification message that could never arrive. Sign-in then refused the account permanently, including the very first one, which is the administrator, leaving the instance unadministrable with hand-written database surgery as the only way back. Sixteen further placeholders had the same shape with milder effects: three sign-in buttons that led nowhere, a checkout that failed at the payment provider, and a browser-session button that was never going to work. All are commented out now, with the reason written where someone copying the file will read it. Two settings were also documented as configurable and could never take effect, because they are baked into the browser bundle when the image is built and were only ever passed at run time.
- [ShieldCheck] **[SECURITY]** **An Invitation Could Be Taken by Someone Who Never Owned the Address**
  Changing your email took effect immediately and left you signed in, without you proving you own the new address. Accepting a team invitation only compared addresses, and because that path never goes through sign-in, the verification requirement sign-in enforces was skipped entirely: claim any address, join any team invited to it. Acceptance now requires a verified address. Two more problems on the same boundary went with it: the pending-invitation list handed a stranger's invitations, team name and inviter included, to anyone who typed their address, and the delete route let an unverified account destroy someone else's invitation.
- [Eye] **[SECURITY]** **An Authenticated Scan That Did Not Finish Showed as Clean**
  The signed-in scan path used the variant of the async layer that discards the record of which branches failed, and its own timeout resolved to an empty result, so a run whose DNS, certificate and live-fetch stages all timed out came back reporting zero findings with nothing to say it was incomplete. The result screen chooses between the partial and the clean verdict from exactly that record, so it drew every-check-ran-and-nothing-fired for a scan that never finished. This is the same fault 3.8.0 fixed on the ordinary and crawl paths, surviving in the one that reaches behind a login, which is the deepest scan offered. Incompleteness is now reported from every failure point, including a lost session, into both the API response and the stored result so history and shared pages agree.
- [Gauge] **[FIXED]** **Form Logins Opened Browser Sessions Nothing Was Paying For**
  Scanning behind a login form starts a real cloud browser, which costs money and is capped per plan. That path checked no allowance, took no concurrency slot and recorded no usage, unlike the two other places that open one. It now does all three, releasing the slot on every exit path and charging only when a session actually came up, so a failed start is never billed and a retry does not pay twice. The account identifier is a required argument now, so a future caller cannot open an unmetered browser by forgetting it.
- [Search] **[FIXED]** **The Checks People Kept Marking Wrong**
  Retuned from real submitted verdicts. The clearest was a parser fault behind the whole Permissions-Policy family: the helper looked for a feature name followed by an optional equals sign and read a missing value as a wildcard, but the older Feature-Policy syntax has no equals sign, so a site that had correctly restricted its camera to itself was reported as granting it to every origin. Others of that kind: the meta-refresh check called working redirects empty because its parser stopped at an inner quote, the source-comment check matched HACK inside Hacker News for want of word boundaries, the session-in-URL check matched the parameter name alone so an ordinary numeric id and a documentation placeholder both fired at high severity, and the base64 check claimed a value decoded to a credential while only testing which characters it used, so every hex digest matched. Two checks were unsound rather than merely loose and are gone: the branch that treated the value of any key named admin or root as a credential, when it is nearly always a route or a class name, and the branch that called a valid content-security policy self-contradictory for using a fallback the way the specification defines it. A third, which flagged any page using a tagged template literal, is removed entirely: a scanner that only reads the response cannot tell whether a template function escapes its inputs, so it fired on every site shipping a mainstream templating library. Checks people had confirmed were narrowed, never gutted.
- [Table2] **[ADDED]** **Engine Feedback Shows What a Check Actually Fired On**
  Every check whose findings come from the asynchronous layer, eighty-seven of them, showed Unknown for category and severity, because those identifiers are built from the finding title and exist in no registry. The description is now the same data the findings themselves are built from, and the type system refuses to compile a new check of that kind without one, so it cannot drift again. The identifiers are unchanged, so every verdict already submitted keeps counting. The rule for flagging a check also hid the worst rows: it flagged a low-severity check three people had confirmed while two critical checks nobody had ever confirmed sat invisible below the sample threshold. Those surface under their own rule now, and the ordering weighs both confidence and severity, so ten reports at sixty percent outrank one report at a hundred. The page also stored which page each finding was on and what the person wrote when dismissing it, and threw both away; expanding a row shows them. There is an export, since reading this page previously meant copying it out by hand.
- [FileText] **[ADDED]** **Findings Show Their Proof, and Triage Reaches Exports**
  The newer checks produce verbatim proof, the exact cookie line, the exact policy directive, the exact matched text, and it was attached to every finding, shipped in the API and fed to the AI verifier while no screen or export ever displayed it. It is shown now, escaped and scrollable, with the invisible characters that survive escaping neutralised so what is printed is what is really there. Marking a finding as accepted risk or a false positive also never left the results page: exports still listed dismissed findings while the summary above them excluded those same findings, so an exported report could print a different verdict from the dashboard for the same scan. Exports tally the header from the findings actually being exported now. The comma-separated export gained thirteen columns it was missing, including the stable finding identifier, and works through the API rather than the browser only. An accepted risk also stops re-sending an alert on every scheduled run.
- [Monitor] **[FIXED]** **The Live Browser Viewer Was Showing the Wrong Shape**
  Sessions are created at a fixed sixteen-by-nine viewport, and nothing else in the app knew that number, so the frame the remote page was drawn into was whatever shape happened to be left over and the session was letterboxed inside it with black bands. That size is one shared setting now, read by both the code that creates the session and the page that displays it, and the frame is measured and fitted to it, so the picture matches the real viewport. The fake browser window drawn around it, complete with a second address bar showing the same address as the real one, is gone, which returns about a hundred pixels of height to the page itself. The network panel remembers whether it is open, and its rows put the full path on its own line instead of clipping long URLs to a stub. The remote-session warning is one compact line that can be dismissed and brought back.
- [Sparkles] **[CHANGED]** **The Assistant Panel Rebuilt**
  The empty state was five full-width suggestion buttons filling most of the panel, the provider name sat in the header as unexplained internal plumbing, and the delete button was one unguarded click next to nothing else. There are three starter prompts now plus a row of real slash commands, the provider moved to the footer where it reads as a note rather than a label, and clearing a conversation asks first. The panel takes the size of the window it is in rather than a fixed height that overflowed short screens, replies are no longer boxed in bubbles that cost thirty pixels of every line of a code block, and the corner radii follow the same ladder as the rest of the product.
- [Wrench] **[FIXED]** **Admin Tables Stopped Bleeding Text Through Their Headers**
  The pinned header on every admin table was drawn at ninety percent opacity with a blur behind it, so rows scrolling underneath showed through it as a smear and text appeared to sit above the bar. A pinned header has to be opaque. The email log was also the only list in the panel that did not cap its own height, so instead of scrolling inside its box it grew the page and pushed everything below it out of view, and the message preview inside it was a fixed height that clipped long messages and left dead space under short ones.
- [Mail] **[CHANGED]** **Every Email Redrawn**
  All sixty-six messages share one layout, and it had drifted into something that looked like a template from a decade ago: a heavy coloured bar across the top of the card, the logo floating above it in dead space, two bordered grey tables that read like a spreadsheet pasted into the message, a warning box edged in a colour that appears nowhere in the product, and the raw browser identification string dumped across two lines. The shell is rebuilt around one spacing scale and one corner ladder, detail rows replace the tables, callouts are tinted the way the application tints them, and the browser string is written as a sentence with the raw value kept small underneath. Ten messages that stacked two grey boxes now show one. Buttons render properly in Outlook, dark mode is handled, and every message stays far under the size at which mail clients truncate.
...
- [FileSearch] **[FIXED]** **A Backup That Could Not Be Read Was Blamed on Its Format**
  Restoring reads the file through two pieces, one that opens it and one that decompresses it, and only the second one's failures were being passed on. So a backup that could not be opened or read at all, a wrong path, a permission, a bad sector, a machine that had run out of file handles, either crashed the process outright or arrived at the reader as a file with nothing in it. That reads as a file we did not write, and the message said so: restore it with psql instead. It sent people to check the format of a file whose real problem was that the disk would not hand it over. The read error is now the error you get, and the one message that can still be reached honestly says how much of the file was read, because a file with lines in it is a pg_dump file and a file with none is a storage problem, and those have opposite fixes.
- [ServerCog] **[ADDED]** **Four Switches for When Something Is Going Wrong**
  There was no way to take the service offline, stop new signups, stop new sign-ins, or stop scanning. The nine existing feature flags decide which features a deployment ships, which is a different question, and a maintenance_mode setting could be written into the database where nothing read it. So when the database went down there was no way to show a maintenance page instead of a server error, and no way to stop scans queueing against it. The admin settings page now opens on an Operations tab with those four switches, each with an optional message shown to the people it affects, because a refusal with no reason is a support ticket. Maintenance mode implies the other three. Every switch is enforced on the server before any work happens, not by hiding a button: scanning is refused at all ten places a scan can start, including the crawl, authenticated, repository, demo and scheduled paths that never touch the ordinary scanner; signups are refused both at the form and at first-time social sign-in, which quietly creates an account as a side effect; and sign-ins are refused after the password is checked rather than before, so the pause cannot be used to work out which addresses have accounts. Staff are exempt from the login pause and from the maintenance page, and the sign-in page stays reachable for everyone, so turning maintenance on cannot lock out the person who has to turn it off. The health endpoint keeps answering, so a container orchestrator does not restart the app in a loop for the duration. For the case the switches cannot cover, the database being down and the admin panel with it, MAINTENANCE_MODE=true in the environment does the same job from the edge, with no database involved.
- [BellRing] **[FIXED]** **Four Ways to Run a Scan Notified Nobody**
  Webhooks, the scan-complete email and the new-findings alert were all wired into one of the five ways a scan can be produced. A crawl, which is what the command line tool and the continuous-integration action run, sent the new-findings alert and nothing else. A scan behind a login, the deepest one offered and the one most likely to turn up something real, and a scan of a connected repository sent nothing at all, so an account could have a webhook registered, watch a scan finish on screen, and never see a delivery. All five run the same block now, and it is one shared piece of code rather than a copy per path, because a copied send loop drifting is exactly how the gap opened. A repository scan reports the repository instead of inventing a web address it does not have, and a scan that did not finish says so in the notification rather than reporting no findings, which is the same distinction the result screen draws: not looked at is not the same as looked at and clean. A receiver that fails, or a mail server that times out, is recorded and dropped, never allowed to fail the scan that produced it.
- [Share2] **[ADDED]** **Alerts That Say What Changed, Not Just How Many**
  The only webhook event was a scan finishing, and its body carried severity counts and no findings, so anything routing an alert to a person had to make a second request to work out what had actually changed. There is a second event now, sent straight after the first whenever a scan turns up a critical or high finding that was not in the previous scan of the same target. It carries those findings, and the ones still outstanding, each with the stable identifier the rest of the product uses, so a chat alert can name the problem in the message and deduplicate on repeats. It is the same comparison the email alert has always run, which means a finding already marked a false positive, an accepted risk, or one nobody intends to fix does not count as new, so a long-standing problem on an hourly schedule still does not alert every hour.
- [Timer] **[SECURITY]** **Seven More Pages That Could Hold the Server Still**
  The scan loop runs every check one after another inside a single synchronous call, and the watchdog that is supposed to stop a slow scan is a timer, so it cannot interrupt a pattern that already has the processor. The demo endpoint runs that same loop with no account required. So each of these was one response, from a host the attacker picks, against the whole process. The worst was the helper that removes documentation blocks before a check reads a page, so a tutorial showing a vulnerable snippet does not report itself as vulnerable. It used the tag-pair pattern that rescans the rest of the document from every opening tag the page never closes: on a page of unclosed code tags, one call took 15 ms at 16 KB and 996 ms at 128 KB, four times the cost for twice the input. It was also being run inside the wrapper around every check in three files, so the same page was stripped 150 times per scan. It is a single forward pass now, and it runs once per page rather than once per check: the same 128 KB takes under 5 ms, and one sweep of all three files went from 4155 ms to 116 ms. Second worst was the pattern for a JSON web token signed with a weak secret, which put optional whitespace, a payload run that also matches whitespace, and more optional whitespace in front of a comma that never arrives, so all three competed for the same characters: 2 KB of spaces took 3573 ms, which puts 4 KB at half a minute and makes the page size limit irrelevant because the payload is tiny. Five more of the same family go with them, including six checks with the same whitespace overlap, one that rescanned the page for a closing form tag from every opening one at 22,775 ms on 512 KB, and one attribute count that paid a 2000 character scan for every angle bracket at 5477 ms on a full-size page. Every one of them now measures under 25 ms on the input that broke it, and every one still fires on a real instance.
- [Activity] **[FIXED]** **The Speed Tests Now Measure the Whole Sweep, Not Just One Check**
  The suite that exists to stop a check becoming catastrophically slow measured each check on its own against a 200 ms line, on a 24 KB page. Two of the defects above were structurally invisible to it. The documentation strip took 33 ms per call at that size, comfortably inside the line, and only became minutes of stalled server because it ran 150 times; and a rescanning loop or a bounded pattern with a large constant needs a bigger page before it separates from an honest one at all. There are three new measurements. A whole-sweep budget runs every check once over one page and bounds the total, which is the only way work repeated per check shows up. A second per-check pass runs at 256 KB, where a rescanning loop and a large constant do separate. The page shapes the corpus never had are in it now: unclosed code, pre, kbd and samp blocks, and an anchor word followed by one long run of the characters the pattern after it also matches, which is the shape that makes a single occurrence quadratic. Running the old code against the new tests fails them at 5.7, 8.4 and 10.7 seconds against the one second budget. The larger page also caught five more checks nobody had reported: the content security policy family at about 3.1 seconds each, both database error checks at about 3.1 seconds, and two link checks at about 1.1 seconds. All are fixed in the same shared helper, and every changed check has a test proving it still detects what it is for.
...
- **Total releases:** 70
- **Total changes documented:** 756
- **Latest:** v3.8.5 (September 6, 2026) - Whose Domain Is It

...
The 860+ checks a scan runs, grouped into 18 categories. Not a marketing number: most have a real page here that tells you what the check catches, why it matters, and how to fix it with code you can paste. The rest only fire across several pages at once, so there is no single check to link to.
...
860+ checks across 18 categories. Paste a domain or an IPv4 address, choose what runs, and read the findings. Nothing to install.
...
Paste a URL, get a security report in under 3 seconds: 860+ deterministic checks across headers, TLS, cookies, DNS, and secrets. No agent to install.

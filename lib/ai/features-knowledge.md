...
_Auto-compiled from the routes under `app/` on 2026-09-08._
...
The 905+ checks a scan runs, grouped into 18 categories. Not a marketing number: most have a real page here that tells you what the check catches, why it matters, and how to fix it with code you can paste. The rest only fire across several pages at once, so there is no single check to link to.
...
905+ checks across 18 categories. Paste a domain or an IPv4 address, choose what runs, and read the findings. Nothing to install.
...
Paste a URL, get a security report in under 3 seconds: 905+ deterministic checks across headers, TLS, cookies, DNS, and secrets. No agent to install.
...
Create a free VulnRadar account. 25 scans a day, full API access, and scan history kept for as long as your account is open. No card required.

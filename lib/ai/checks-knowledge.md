...
export async function POST(req: Request) {
  ...
  if (files.length > MAX_FILES) {
    return Response.json({ error: `Maximum <value> files allowed` }, { status: 400 });
  }

  const totalSize = files.reduce((sum, f) => sum + f.size, 0);
  if (totalSize > MAX_TOTAL_SIZE) {
    return Response.json({ error: "Total upload size too large" }, { status: 400 });
  }
  // process files
}
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Timing-Allow-Origin", value: "https://analytics.yoursite.com" }] }];
  },
};
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{
      key: "Content-Security-Policy",
      value: "upgrade-insecure-requests; default-src 'self'; script-src 'self'; object-src 'none';"
    }]}];
  },
};
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Accept-CH", value: "Sec-CH-UA-Mobile, Sec-CH-UA-Platform" }] }];
  },
};
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [
      ...
      { key: "Permissions-Policy", value: "ch-ua-mobile=(), ch-ua-platform=()" },
    ]}];
  },
};
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Speculation-Rules", value: "https://yoursite.com/speculation-rules.json" }] }];
  },
};
```
...
<script type="speculationrules">
{
  ...
  "prefetch": [
    {
      "where": { "and": [{ "href_matches": "/*" }, { "not": { "href_matches": "/checkout/*" }}] },
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Accept-CH", value: "Sec-CH-UA-Arch" }] }];
  },
};
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Accept-CH", value: "Sec-CH-UA-Bitness" }] }];
  },
};
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Accept-CH", value: "Sec-CH-UA-Model" }] }];
  },
};
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Accept-CH", value: "Sec-CH-UA-Platform-Version" }] }];
  },
};
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [
      { key: "Report-To", value: JSON.stringify({ group: "nel", max_age: 86400, endpoints: [{ url: "https://yoursite.com/api/reports/nel" }] }) },
      { key: "NEL", value: JSON.stringify({ report_to: "nel", max_age: 86400, include_subdomains: true }) },
    ]}];
  },
};
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{ key: "Document-Policy", value: "no-synchronous-xhr" }] }];
  },
};
```
...
export default {
  async headers() {
    return [{ source: "/(.*)", headers: [{
      key: "Report-To",
      value: JSON.stringify({ group: "default", max_age: 86400, endpoints: [{ url: "https://yoursite.com/api/reports" }] })
    }]}];
  },
};
```
...
- **Link with SRI** (html):
```html
<link rel="stylesheet" href="https://cdn.example.com/lib.min.css" integrity="sha384-HASH" crossorigin="anonymous" />
```
...
**Sec-Fetch-* headers not validated server-side**
...
<div style="position: relative">
  ...
  <button type="button" onclick="var i=document.getElementById('pw'); i.type=i.type==='password'?'text':'password'" aria-label="Toggle password visibility">
    Show
  </button>
</div>
```
...
```html
<input type="tel" name="phone" autocomplete="tel" placeholder="+1 (555) 000-0000" />
```

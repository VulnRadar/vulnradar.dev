 ...
 *
 * The passive-source aggregation + DNS brute-force + reachability pipeline
 * that powers subdomain discovery. Extracted verbatim from the POST
 * /api/v3/scan/discover route so it can be driven from two callers with
 * identical behavior:
 *
 *   1. The manual route (app/api/v3/scan/discover/route.ts), which keeps all
 *      of its auth / rate-limit / SSRF / input validation and simply calls
 *      discoverSubdomainsForRoot() on a cache miss.
 *   2. The automatic in-scan capture (lib/scanner/subdomain-auto.ts), which
 *      runs the same engine best-effort during every scan so the subdomain
 *      panel is populated without the user pressing "Discover".
 *
 * Both share the same per-domain DB cache (subdomain_cache), so a domain
 * discovered once -- by either path -- is served instantly to the other.
 *
 * dns/promises is imported as the default export (not a namespace) to match
 * the discover route's original import style and its test's module mock.
 */
...
export async function getCachedSubdomains(
  domain: string,
...
/**
 * Run the full discovery pipeline for an already-validated registrable
 * domain and return the response body. Assumes the caller has already done
 * auth / rate-limiting / SSRF validation of the originating URL -- this is
 * the shared core, not the request boundary.
 *
 * `requestId` (optional) drives the live progress side channel
 * (lib/scanner/discovery-progress.ts) for the manual flow's progress bar;
 * the auto path passes none.
 */
export async function discoverSubdomainsForRoot(
  rootDomain: string,

...
const detectors: Record<string, DetectFn> = {
  ...
  "hardcoded-secrets": (_url, _headers, body) => {
    // Skip check on very short bodies (likely error pages) or bodies that look like documentation
    const lowerBody = body.toLowerCase()
    const isDocPage = lowerBody.includes("documentation") && lowerBody.includes("example") && lowerBody.includes("api")

    const patterns = [
      // ── Cloud Providers (high-confidence prefixed patterns) ─────
      ...
      // ── Payment Providers (high-confidence prefixed patterns) ───
      ...
      // ── Version Control & CI (high-confidence prefixed) ────────
      ...
      // ── Communication (high-confidence prefixed) ───────────────
      ...
      { name: "Discord Bot Token", pattern: /[MN][A-Za-z\d]{23,}\.[\w-]{6}\.[\w-]{38,}/g },
      ...
      // ── Email Services (high-confidence prefixed) ──────────────
      { name: "SendGrid Key", pattern: /SG\.[0-9A-Za-z_-]{22}\.[0-9A-Za-z_-]{43}/g },
      { name: "Mailgun Key", pattern: /key-[0-9a-f]{32}/g },

      // ── Database URIs (contain credentials in plaintext) ───────
      { name: "MongoDB URI", pattern: /mongodb(?:\+srv)?:\/\/[^:]+:[^\s@"'<>]+@[^\s"'<>]{5,}/g },
      { name: "PostgreSQL URI", pattern: /postgres(?:ql)?:\/\/[^:]+:[^\s@"'<>]+@[^\s"'<>]{5,}/g },
      { name: "MySQL URI", pattern: /mysql:\/\/[^:]+:[^\s@"'<>]+@[^\s"'<>]{5,}/g },
      { name: "Redis URI", pattern: /rediss?:\/\/[^:]+:[^\s@"'<>]+@[^\s"'<>]{5,}/g },
      { name: "Firebase URL", pattern: /https:\/\/[a-z0-9-]+\.firebaseio\.com/g },

      // ── Auth & Identity (high-confidence prefixed) ─────────────
      { name: "OAuth Token", pattern: /ya29\.[0-9A-Za-z_-]{68,}/g },

      // ── AI & ML (high-confidence prefixed) ─────────────────────
      ...
      // ── Analytics (high-confidence prefixed) ───────────────────
      ...
      // ── Social (high-confidence) ───────────────────────────────
      { name: "Facebook Token", pattern: /EAA[0-9A-Za-z]{100,}/g },

      // ── Private Keys (definitive, zero false positives) ────────
    ...
    // Skip entirely on obvious documentation/example pages
    ...
    for (const { name, pattern } of patterns) {
      ...
      if (matches) {
        ...
        const unique = [...new Set(matches)].filter((m) => {
          ...
          if (lower.includes("example") || lower.includes("your_") || lower.includes("xxxx") || lower.includes("0000")) return false
          if (lower.includes("placeholder") || lower.includes("test_") || lower.includes("dummy")) return false
          // Skip localhost database URIs (development examples)

...
const detectors: Record<string, DetectFn> = {
  ...
  "debug-indicators": (_url, headers, body) => {
    ...
    if (hasHeader(headers, "x-debug-token-link"))
  ...
  "cookie-no-secure-prefix": (_url, headers) => {
    ...
    return noPrefixed.length > 0
      ? `${noPrefixed.length} sensitive cookie(s) lack __Host- or __Secure- prefix: ${noPrefixed.map((c) => c.split("=")[0]?.trim()).join(", ")}`
      : null;
  },
  ...
  "cookie-max-age-excessive": (_url, headers) => {
    ...
    for (const c of cookies) {
      ...
      if (m) {
        ...
        if (secs > 31536000)
          return `Cookie '${c.split("=")[0]}' has max-age of ${Math.round(secs / 86400)} days.`;
      }
    }
    return null;
  },
  ...
  "session-cookie-flags": (_url, headers) => {
    ...
    if (/session|sid|auth/i.test(setCookie)) {
      ...
      if (issues.length > 0)
        return `Session cookie has issues: ${issues.join(", ")}.`;
    }
    return null;
  },

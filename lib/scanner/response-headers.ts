/**
 * SECURITY-AUDIT-2026-06-28 / M-10: filter out sensitive response headers
 * before persisting them to scan_history.response_headers.
 *
 ...
 * the full plan retention window (up to indefinite for paid plans).
 * A read-only DB compromise yielded every scan-target session token
 * ever issued.
 *
 * This helper returns a copy of the headers map with sensitive keys
 * redacted (replaced with `"[redacted]"`). The redact list is
 * case-insensitive and is enforced server-side.
 */

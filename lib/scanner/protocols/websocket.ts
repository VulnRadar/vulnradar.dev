 ...
 */
export function runWebSocketChecks(
  url: string,
  headers?: Headers,
): Vulnerability[] {
  ...
  if (headers) {
    ...
    if (allowOrigin === "*") {
      findings.push({
        ...
        riskImpact:
          "Any website can establish WebSocket connections to this endpoint.",
        explanation:
          "Consider restricting allowed origins for WebSocket connections.",
        fixSteps: [
    ...
    if (extensions?.includes("permessage-deflate")) {
      findings.push({
        ...
        riskImpact:
          "Compression combined with secrets in messages may leak data.",
        explanation:
          "Compression side-channel attacks like CRIME can reveal secrets.",
        fixSteps: [

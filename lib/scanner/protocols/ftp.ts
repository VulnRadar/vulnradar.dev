...
export function runFtpChecks(url: string): Vulnerability[] {
  ...
  if (!isSecure) {
    findings.push({
      ...
      explanation:
        "FTP has no encryption. Any network observer can see passwords and file contents.",
      fixSteps: [
...
lftp -u user,password ftps://example.com`,
  ...
  findings.push({
    ...
    explanation:
      "FTP servers cannot be scanned like HTTP. Full security assessment requires FTP client tools.",
    fixSteps: [

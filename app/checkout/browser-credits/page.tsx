...
import {
  ...
  type BrowserbaseCreditTier,
} from "@/lib/billing/browserbase-credit-catalog";
import { BrowserbaseCreditCheckout } from "@/components/billing/browserbase-credit-checkout";
import { CheckoutSkeleton } from "@/components/billing/checkout-skeleton";

function minutesPerDollar(tier: BrowserbaseCreditTier): number {
  return tier.minutes / (tier.priceInCents / 100);
}

export default function BrowserCreditsCheckoutPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [selectedTier, setSelectedTier] =
    useState<BrowserbaseCreditTier | null>(null);
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        if (!meRes.ok) {
          router.push(`${ROUTES.LOGIN}?redirect=/checkout/browser-credits`);
          return;
        }
      } catch {
        router.push(`${ROUTES.LOGIN}?redirect=/checkout/browser-credits`);
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center max-w-md px-4">
          ...
          <p className="text-muted-foreground mb-4">
            Billing is switched off on this {APP_NAME} deployment, so
            live-browser sessions have no monthly cap to top up.
          </p>
      ...
      >
        <div className="text-center px-4 max-w-md">
          ...
          <h1 className="text-2xl font-bold mb-2">
            {purchaseResult.minutes.toLocaleString()} minute
            {purchaseResult.minutes === 1 ? "" : "s"} added
          </h1>
          <p className="text-muted-foreground mb-6">
            Your purchased balance is now{" "}
            <span className="font-medium text-foreground">
              {(purchaseResult.balanceSeconds / 60).toLocaleString()}
            </span>{" "}
            minutes. They never expire and are spent only once your plan&apos;s
            free monthly allowance runs out.
          </p>
  ...
  const baseRate = minutesPerDollar(BROWSERBASE_CREDIT_TIERS[0]);
  const baseTierPrice = (
    BROWSERBASE_CREDIT_TIERS[0].priceInCents / 100
  ).toFixed(0);
      ...
      <main className="container max-w-4xl mx-auto px-4 py-8 md:py-12">
        <div className="mb-8 max-w-2xl">
          <h1 className="text-2xl md:text-3xl font-bold mb-2">
            Buy more live-browser minutes
          </h1>
          <p className="text-muted-foreground">
            A one-time top-up on top of your plan&apos;s free monthly allowance.
            Minutes never expire and only get spent once that free allowance
            ...
            {BROWSERBASE_CREDIT_TIERS.map((tier) => {
              const isBest =
                ...
                BROWSERBASE_CREDIT_TIERS[BROWSERBASE_CREDIT_TIERS.length - 1]
                  .id;
              const multiplier = minutesPerDollar(tier) / baseRate;
              return (
                <button
                  key={tier.id}
                  type="button"
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {multiplier === 1
                          ? `${minutesPerDollar(tier).toFixed(1)} minutes per dollar`
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 md:items-start max-w-4xl">
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                <div className="flex items-baseline justify-between gap-3">
                  <p className="font-semibold text-lg">
                    {selectedTier.minutes.toLocaleString()} minutes
                  </p>
            ...
            <div>
              <div className="rounded-xl border border-border bg-card p-6">
                ...
                <BrowserbaseCreditCheckout
                  tier={selectedTier}
                  onSuccess={(minutes, balanceSeconds) =>
                    setPurchaseResult({ minutes, balanceSeconds })
                  }
                />
              </div>

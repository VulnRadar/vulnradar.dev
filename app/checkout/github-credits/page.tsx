...
import { GithubCreditCheckout } from "@/components/billing/github-credit-checkout";
import { CheckoutSkeleton } from "@/components/billing/checkout-skeleton";

function tokensPerDollar(tier: GithubCreditTier): number {
  return tier.tokens / (tier.priceInCents / 100);
}

export default function GithubCreditsCheckoutPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [selectedTier, setSelectedTier] = useState<GithubCreditTier | null>(
    null,
  );
  ...
  const baseRate = tokensPerDollar(GITHUB_CREDIT_TIERS[0]);
  const baseTierPrice = (GITHUB_CREDIT_TIERS[0].priceInCents / 100).toFixed(0);
      ...
      <header className="border-b border-border bg-card/50 sticky top-[var(--vr-banner-h,0px)] z-10 transition-[top] duration-300">
        <div className="container max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-md border border-border/60 bg-muted/40 px-2.5 py-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
          >
      ...
      <main className="container max-w-4xl mx-auto px-4 py-8 md:py-12">
        <div className="mb-8 max-w-2xl">
          ...
          <p className="text-muted-foreground">
            ...
            for GitHub repo AI review. Tokens never expire and only get spent
            once that free allowance runs out for the window.
          </p>
        </div>
        ...
        {!selectedTier ? (
          <div className="max-w-2xl rounded-xl border border-border bg-card divide-y divide-border overflow-hidden">
            {GITHUB_CREDIT_TIERS.map((tier) => {
              const isBest =
                tier.id ===
                GITHUB_CREDIT_TIERS[GITHUB_CREDIT_TIERS.length - 1].id;

  ...
  try {
    ...
    return NextResponse.json({
      ...
      sensitiveData: {
        ...
        paymentMethod: paymentMethod
          ? {
              ...
              cardSupports3dSecure:
                paymentMethod.three_d_secure_usage?.supported || false,
              cvvCheck: paymentMethod.card?.checks?.cvc_check || null,
              postalCodeCheck:
                paymentMethod.card?.checks?.postal_code_check || null,
              addressLine1Check:
                paymentMethod.card?.checks?.address_line1_check || null,
        ...
        scansPerDay: item?.metadata?.scansPerDay
          ? parseInt(item.metadata.scansPerDay)
          : null,
        ...
        nextBillingDate: item?.current_period_end
          ? new Date(
              (item.current_period_end +
                (item.billing_thresholds?.usage_gte || 0)) *
                1000,
            ).toISOString()
          : null,
        ...
        // Dates
        customerCreatedAt: customer?.created
        ...
        isOnTrial: subscription.trial_end

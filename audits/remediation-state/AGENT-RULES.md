...
This is the SECOND remediation pass. 248 findings were already fixed and their
ids are excluded from your file, so everything in it is genuinely open as far
as the audit knows. Some will still turn out already-fixed: several audits are
months old and later work closed things without updating the record. Verify
first, every time.
...
reason: it needs a file you do not own, it is a page redesign, or it is a
